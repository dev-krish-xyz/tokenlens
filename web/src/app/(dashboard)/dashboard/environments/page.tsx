...
const STATUS_CHIP: Record<string, { color: string }> = {
  Active: { color: '#10B981' },
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
        {envs.map((env) => {
          ...
          return (
            <div key={env.id} style={{ background: 'var(--surface)', borderRadius: 12, padding: 18 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div style={{ fontWeight: 600 }}>{env.name}</div>
                <span style={{ fontSize: 11, color: chip.color, fontWeight: 500 }}>{env.status}</span>
              </div>
        ...
        <div
          onClick={() => { /* TODO: trpc.environments.create.useMutation() */ }}
          style={{ background: 'var(--surface)', border: '1px dashed #E5E5E5', borderRadius: 12, padding: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 160, cursor: 'pointer' }}
      ...
      {/* Info box */}
      <div style={{ background: 'var(--surface)', borderRadius: 12, padding: '14px 16px', marginTop: 20, fontSize: 12, color: 'var(--t2)', lineHeight: 1.6 }}>
