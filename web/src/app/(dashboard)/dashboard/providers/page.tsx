    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
        {EXAMPLE_PROVIDERS.map((p) => (
          <div key={p.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderLeft: '3px solid #16A34A', borderRadius: 14, padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 15 }}>{p.name}</div>
                <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 2, fontFamily: 'monospace' }}>{p.url}</div>
              </div>
              <span style={{ display: 'inline-flex', padding: '2px 8px', borderRadius: 9999, fontSize: 10, fontWeight: 500, background: '#DCFCE7', color: '#16A34A' }}>{p.status}</span>
            </div>
        ...
        <div
          ...
          style={{ background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: 14, padding: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200, cursor: 'pointer' }}
