...
- `alert:sent:{virtualKeyId}:{configId}:{YYYYMMDDHH}` — per-key alert dedup, TTL = config cooldown
- `alert:sent:ws:{workspaceId}:{configId}:{YYYYMMDDHH}` — workspace alert dedup, TTL = config cooldown
- `alert:sent:anomaly:{virtualKeyId}:{configId}:{YYYYMMDDHH}` — anomaly alert dedup, TTL = config cooldown
